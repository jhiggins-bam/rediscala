...
class ClusterCommandRedisClusterTest extends RedisClusterClients {
  ...
  "tools" should {
    "groupby" in {
      redisCluster.groupByClusterServer(Seq("{foo1}1","{foo2}1","{foo1}2","{foo2}2")).sortBy(_.head).toList mustEqual( Seq(Seq("{foo2}1","{foo2}2"),Seq("{foo1}1","{foo1}2")).sortBy(_.head)  )
    }
  }
