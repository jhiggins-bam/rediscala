...
The codebase has been benchmarked with [ScalaMeter](http://scalameter.github.io/). According to the results
it's currently possible to perform more than 250,000 requests per second.

The full result can be viewed [here](http://bit.ly/rediscalabench-1-1).

The benchmarks can be executed with the following commands:

```
nix-shell --run "sbt bench/test"
chromium rediscala/tmp/report/index.html
```
